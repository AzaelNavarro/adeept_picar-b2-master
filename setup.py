...
service_content = """
[Unit]
Description=Adeept Robot Web Server
...
""".format(
    script_path=thisPath + "/web/webServer_HAT_V3.1.py",
    working_dir=thisPath + "/web",
    user=username
)

try:
    with open("/etc/systemd/system/adeept_robot.service", "w") as f:
        f.write(service_content)

    os.system("sudo systemctl daemon-reexec")
    os.system("sudo systemctl daemon-reload")
    os.system("sudo systemctl enable adeept_robot.service")
    os.system("sudo systemctl start adeept_robot.service")
except Exception as e:
    print("Error creando el servicio systemd:", e)
